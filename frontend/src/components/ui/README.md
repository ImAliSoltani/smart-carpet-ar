...
| `compare-reveal.tsx` | [rmahammad/compare-reveal](https://21st.dev/@rmahammad/components/compare-reveal) — بلوک توکن `--motiq-*` و پالت تیره‌اش کامل حذف شد؛ شاخه‌ی `{src, alt}` که خودش `<img>` می‌ساخت برداشته شد چون تنها مصرف‌کننده `<picture>` هنرِ‌کارگردانی‌شده می‌دهد. **دو تغییر رفتاری:** (۱) لمس دیگر نمی‌پرد — `pointerdown` روی کل کادر بلافاصله جداکننده را جابه‌جا می‌کرد و روی گوشی اولین تماسِ یک اسکرول عمودی عکس را نصف می‌کرد؛ حالا انگشت باید ۶ پیکسل افقی (و بیشتر از عمودی) حرکت کند. (۲) سوییپِ معرفی دیگر خفه نمی‌شود — شاخه‌ی «انیمیشن نکن» هر دو دلیلِ «کم‌حرکتی» و «بیرون از دید» را یکی می‌گرفت و سوییپ را *انجام‌شده* علامت می‌زد؛ چون این کادر پشت پرده‌ی ورودی mount می‌شود، یعنی هیچ‌وقت اجرا نمی‌شد |
| `elegant-carousel.tsx` | [dev.yadhakim/elegant-carousel](https://21st.dev/@dev.yadhakim/components/elegant-carousel) — پاسخ رجیستری کلاس‌های `carousel-*` را از یک `index.css` می‌خواند که همراهش نیامد، پس تمام استایل با توکن‌های خودمان نوشته شد. **دکمه‌ی توقف اضافه شد** (نسخه‌ی اصلی فقط با هاور می‌ایستاد، که صفحه‌کلید و گوشی را رها می‌کند)، `prefers-reduced-motion` هم پخش خودکار و هم زوم کن‌برنز را خاموش می‌کند، `aria-live` هنگام چرخش خودکار خاموش می‌ماند، جهت سوایپ و فلش‌ها به `dir` بسته شد، و دو `setInterval` موازی (نوار پیشرفت و نوبت) به یک حلقه‌ی `requestAnimationFrame` تبدیل شد چون از هم جدا می‌افتادند |
...
| `lib/ar/carpet-placer.js` | **از ۲۱st نیست و نباید باشد** — نقشه‌ی راه این لایه را صریحاً دست‌ساز و بیرون از دسترس کاتالوگ‌ها گذاشته، چون هیچ کتابخانه‌ای فرش را فرش نمی‌بیند. از `spike/carpet-placer.js` منتقل شد و **فقط دو خط import‌ش عوض شد**؛ همان کدی است که روی گوشی واقعی تأیید شده، و بازنویسی‌اش یعنی برگرداندن تنها بخش اثبات‌شده‌ی پروژه به ستون اثبات‌نشده در ازای تایپ. سطح عمومی‌اش در `carpet-placer.d.ts` دستی اعلام شده |
...
| `toranjan/home-hero.tsx` | چیدمانش دست‌ساز است، ولی ماشینش `ui/compare-reveal.tsx` بالاست. دو جفت عکسِ هنرِ‌کارگردانی‌شده (۳:۲ افقی و ۴:۵ عمودی) با `<picture>` و یک `media` عوض می‌شوند — نه `next/image`، چون این دو *عکس متفاوت‌اند* نه یک عکس در دو اندازه. نقطه‌ی شکست ۱۰۲۴ در `<source>` و در `lg:` نسبت کادر **باید یکی بمانند** |
| `toranjan/home-showcase.tsx` | کاروسل بالا را به کاتالوگ وصل می‌کند. فرش‌ها با یک *قاعده* انتخاب می‌شوند («گران‌ترین ابریشم و پشم») نه با فهرست اسلاگ، وگرنه صفحه‌ی اول روزی می‌شکند که کسی فرشی را از پنل بردارد. دو دور واکشی: فهرست چهارتا را می‌چیند و جزئیاتِ هرکدام متنِ توصیفی و `full_url` را می‌آورد — رنگِ لهجه‌ی هر اسلاید هم `colors[0]` خود فرش است |
