 ...
 *
 * Moved into the storefront from `spike/carpet-placer.js`, which is where it was
 * written and — more to the point — where it was **verified on real hardware**,
 * on Android over WebXR and on iPhone through Quick Look. It is kept as
 * JavaScript and edited only at its two import lines, because a rewrite would
 * put the one part of this project that has been proven on a device back into
 * the unproven column in exchange for type annotations. Its public surface is
 * declared in `carpet-placer.d.ts` instead.
 *
 * The roadmap lists this layer among the pieces that are deliberately
 * hand-built and outside any component catalogue: no library treats a rug as a
 * rug, and everything below is that difference.
 */

/**
 * What makes it carpet-specific.
 *
